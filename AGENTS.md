...
## Mandatory Outbound Email Safety Hold

This is a standing safety gate for every email ChatGPT, Codex, an automation, or another 910CPR agent prepares or transmits on Brian's behalf. See owner instruction [#268](https://github.com/Brian910cpr/910cpr-class-landers/issues/268).

- Never transmit a 910CPR email immediately.
- Resolve the exact sender account, recipients, reply versus reply-all scope, subject, thread, attachments, and body before the hold starts.
- Do not add recipients, broaden scope, invent facts, introduce new claims, or take stylistic liberties without first presenting the proposed change to Brian and waiting for approval.
- Create the Gmail draft first.
- Present the complete proposal to Brian: From, To, CC/BCC, subject, body, attachments, thread/reference, and exact planned send time.
- After presentation, create a one-time send for 20 minutes later.
- During the hold, any objection, stop, pause, requested edit, or indication of distress is an immediate cancellation condition. Preserve the draft; do not transmit it.
- A revised email requires presentation of the revised proposal and a fresh 20-minute hold. Never reuse the original deadline after a material edit.
- If safe delayed transmission is unavailable, fail closed: leave the email as a draft and report that it was not scheduled. Never fall back to immediate send.
- Verify the resulting Gmail message ID and thread after transmission before reporting success.
- Concerns about wording, recipients, scope, missing information, or possible improvements belong before scheduling. Do not send first and then disclose them.
- Do not claim this workflow is `PROVEN`, `MONITORED`, or `HEALTHY` until the evidence requirements in issue #268 and `LANDERWARE_PROOF_AND_HEALTH_STANDARD.md` are satisfied.
