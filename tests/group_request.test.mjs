...
test('a receipt requires both a saved inquiry and a Brian action prompt, with safe retries',async t=>{const db=database(t);const first=await handler(request());assert.equal(first.status,200);const receipt=await first.json();assert.equal(receipt.received,true);assert.equal(db.inquiries.size,1);assert.equal(db.cards.size,1);assert.equal([...db.inquiries.values()][0].page_url,'https://www.910cpr.com/group-training.html');const prompt=boardPrompts([...db.cards.values()]);assert.equal(prompt.length,1);assert.equal(prompt[0].label,'See the exact step');assert.match(prompt[0].title,/Contact Test Office/);assert.match(prompt[0].href,/production\.html\?card=/);const second=await (await handler(request())).json();assert.equal(second.reference,receipt.reference);assert.equal(db.inquiries.size,1);assert.equal(db.cards.size,1);});
