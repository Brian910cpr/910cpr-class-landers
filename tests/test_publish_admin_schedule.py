...
class PublishAdminScheduleTest(unittest.TestCase):
    def test_durable_little_leaps_is_projected_with_canonical_count(self) -> None:
        ...
        self.assertEqual("AHA Heartsaver Pediatric First Aid CPR AED", row["course_name"])
        self.assertEqual("5226 S College Rd, Wilmington", row["location_name"])
        self.assertEqual("Brian Ennis", row["lead_instructor_name"])
        self.assertEqual(0, row["registered_count"])
    ...
    def test_event_identity_dedupes_durable_and_enrollware_without_external_id(self) -> None:
        payload = {"sessions": [{"session_id": "ew-456", "start_at": "2026-09-19T11:00:00-04:00", "course_name": "AHA Heartsaver Pediatric First Aid CPR AED", "location_name": "Little Leaps"}]}
        durable = {"available": True, "sessions": [{"id": "durable-2", "start_at": "2026-09-19T11:00:00-04:00", "course_name": "AHA Heartsaver Pediatric First Aid CPR AED", "location_name": "Little Leaps", "lead_instructor_name": "Brian Ennis", "registered_count": 0}]}
        result = build_admin_schedule(payload, now=datetime.fromisoformat("2026-08-30T08:00:00-04:00"), durable_snapshot=durable)
        self.assertEqual(1, len(result["sessions"]))
        self.assertTrue(result["sessions"][0]["durable_session"])
        self.assertEqual(1, result["counts"]["durable_external_matches"])
