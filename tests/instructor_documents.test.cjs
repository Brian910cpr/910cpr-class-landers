...
function runtime({authenticated=true,document=doc,rpc={ok:true,removed_id:docId},storageStatus=200}={}) {
  ...
  const context = vm.createContext({Request,Response,Headers,URL,TextEncoder,crypto,File,console:{error(){}},
    Deno:{env:{get:n=>n==='SUPABASE_URL'?base:'server-test-key'},serve:fn=>handler=fn},
    fetch:async(url,options={})=>{
      calls.push({url,options});
      if(url.includes('/rest/v1/maxim_portal_sessions?'))return Response.json(authenticated?[{token_sha256:'hashed'}]:[]);
  ...
  let source=fs.readFileSync('supabase/functions/instructor-workbench/documents.ts','utf8').replaceAll('export ','');
  source+='\n'+fs.readFileSync('supabase/functions/instructor-workbench/index.ts','utf8').replace(/^import .*\n/,'');
  vm.runInContext(stripTypeScriptTypes(source),context);
  const send=(method='GET',body,headers={'x-maxim-session':'test-token'},path=`sessions/${session}/documents/${docId}`)=>handler(new Request(`${base}/functions/v1/instructor-workbench/${path}`,{method,headers,...(body!==undefined?{body:JSON.stringify(body)}:{})}));
  return {send,calls};
}
...
test('expired or revoked login stops before document queries',async()=>{
  const r=runtime({authenticated:false});for(const method of ['GET','DELETE'])assert.equal((await r.send(method,method==='DELETE'?{confirm:true}:undefined)).status,401);assert.ok(r.calls.every(c=>c.url.includes('maxim_portal_sessions')));
});
...
test('confirmed removal passes only exact IDs and server-verified actor to atomic RPC',async()=>{
  ...
  const body=JSON.parse(r.calls[1].options.body);assert.equal(body.p_class_session_id,session);assert.equal(body.p_document_id,docId);assert.match(body.p_actor_session_hash,/^[a-f0-9]{64}$/);assert.equal(Object.keys(body).length,3);assert.equal(r.calls.length,2);
});
...
test('repeat removal returns success and invalid IDs cannot alter filters',async()=>{
  ...
  assert.equal((await r.send('DELETE',{confirm:true},{'x-maxim-session':'test-token'},`sessions/${session}/documents/not-a-uuid`)).status,404);
});
