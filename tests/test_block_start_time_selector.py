...
class BlockStartTimeSelectorTests(unittest.TestCase):
    ...
    def test_rendered_html_loads_shell_without_actionable_static_times(self):
        ...
        self.assertIn("const tolerance = 4", html)
        self.assertIn("const endTolerance = Math.max(tolerance, Math.min(24, list.clientWidth * 0.06))", html)
        self.assertIn("previous.disabled = !hasOverflow || atStart", html)
        self.assertIn("next.disabled = !hasOverflow || atEnd", html)
    ...
    def test_config_driven_heartsaver_page_generates_valid_schedule(self):
        ...
        self.assertIn("day-star", html)
        self.assertIn("is-anchor", html)
    ...
    def test_shared_schedule_role_visual_language_is_semantic_and_compact(self):
        ...
        self.assertNotIn("Scheduled · Anchor", html)
        self.assertNotIn("Attached · Barnacle", html)
        self.assertIn("★ Scheduled class", html)
        self.assertNotIn("scheduled class; anchor", html)
        self.assertNotIn("barnacle; attached opportunity around an anchor", html)
    ...
    def test_hsi_selector_shell_uses_resolved_authority_without_static_times(self):
        ...
        self.assertIn("Choose the HSI class that fits your requirement, then select a date and time.", html)
        self.assertIn('"courseId": "445670"', html)
        self.assertIn('"deliveryMode": "blended"', html)
        self.assertIn('"deliveryBadge": "BLENDED"', html)
        self.assertIn("Complete the assigned online coursework first, then attend the in-person skills session.", html)
        self.assertIn('<div class="delivery-help-label">BLENDED LEARNING</div>', html)
        self.assertIn("Complete the course at our Wilmington training center.", html)
        self.assertNotIn("REQUEST OPTIONS", html)
        self.assertNotIn("Other options", html)
        self.assertNotIn("validated public instant booking", html)
        self.assertNotIn("request action", html)
        self.assertNotIn("instructor review", html)
        self.assertNotIn("not connected to instant online booking", html)
        self.assertNotIn("unsupported formats", html)
        self.assertNotIn("Combined HSI BLS and Adult First Aid option completed in person.", html)
        self.assertNotIn("coastalcprtraining.enrollware.com/enroll?appointmentDayId", html)
        offers = artifact_offers(artifact)
        self.assertGreater(len(offers), 0)
    ...
    def test_hsi_bls_adult_first_aid_delivery_labels_are_consistent(self):
        config = json.loads((ROOT / "data" / "config" / "block_schedule_pages.json").read_text(encoding="utf-8"))
        hsi_options = config["pages"]["hsi"]["course_options"]
        option = next(item for item in hsi_options if item["course_id"] == "445670")
        challenge = next(item for item in hsi_options if item["course_id"] == "463743")
        self.assertEqual("HSI BLS + Adult First Aid", option["display_label"])
        self.assertEqual("blended", option["delivery_mode"])
        self.assertEqual("BLENDED", option["delivery_badge"])
        self.assertNotIn("completed in person", option["clarification"].lower())
        self.assertIn("online coursework", option["clarification"].lower())
        self.assertEqual("in-person", challenge["delivery_mode"])
        self.assertEqual("IN-PERSON", challenge["delivery_badge"])

        catalog = json.loads((ROOT / "data" / "config" / "course_catalog.json").read_text(encoding="utf-8"))
        course = next(item for item in catalog["courses"] if item["course_id"] == "445670")
        self.assertEqual("HSI BLS and Adult First Aid | Blended Learning", course["official_title"])
        self.assertEqual("BL", course["delivery_type"])
        self.assertEqual("blended", course["blended_classroom_skills"])

        artifact = json.loads((ROOT / "docs" / "data" / "block-selector-availability" / "hsi.json").read_text(encoding="utf-8"))
        hsi_offers = [offer for offer in artifact_offers(artifact) if offer["courseId"] == "445670"]
        self.assertGreater(len(hsi_offers), 0)
        self.assertEqual({"blended"}, {offer["deliveryMode"] for offer in hsi_offers})
    ...
    def test_course_rail_controls_have_accessible_labels_and_overflow_logic(self):
        ...
        self.assertIn("const tolerance = 4", html)
        self.assertIn("const endTolerance = Math.max(tolerance, Math.min(24, list.clientWidth * 0.06))", html)
        self.assertIn("const atStart = list.scrollLeft <= tolerance", html)
        self.assertIn("const atEnd = list.scrollLeft >= maxScroll - endTolerance", html)
        self.assertIn("previous.disabled = !hasOverflow || atStart", html)
        self.assertIn("next.disabled = !hasOverflow || atEnd", html)
        self.assertIn("scheduleCourseRailUpdate()", html)
        self.assertIn(".course-rail:not(.has-overflow) .rail-arrow", html)
        self.assertIn("list.scrollTo({ left: target, behavior: 'smooth' })", html)
        self.assertIn("padding: 0 52px", html)
        self.assertIn("padding: 0 0 10px", html)
        self.assertIn("top: 56px", html)
        self.assertIn("height: 118px", html)
