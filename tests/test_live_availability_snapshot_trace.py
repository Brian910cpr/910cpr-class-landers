...
class LiveAvailabilitySnapshotTraceTest(unittest.TestCase):
    ...
    def test_seed_simulation_august_bls_reaches_active_generation(self) -> None:
        self.assertGreater(self.summary["seed_simulation_august_bls_blocks"], 0)
        self.assertGreater(self.summary["live_snapshot_august_blocks"], 0)
        self.assertGreater(self.summary["dynamic_august_offers"], 0)
        self.assertTrue(self.summary["source_mismatch_resolved"])
        self.assertEqual("resolved_after_rrule_expansion", self.summary["first_divergence_file"])
    ...
    def test_runtime_rrule_expansion_is_visible(self) -> None:
        self.assertEqual(0, self.summary["runtime_events_with_august_rrule_not_expanded"])
        self.assertGreater(self.summary["runtime_august_events_generated_from_rrule"], 0)
        self.assertGreater(self.summary["runtime_august_events"], 0)

    def test_course_master_is_downstream_of_trace(self) -> None:
        self.assertIn("dynamic generation", self.summary["zero_reason"])
        self.assertIn("before Course Master", self.summary["zero_reason"])
