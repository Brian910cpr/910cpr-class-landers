...
def guess_course_family(text: str) -> str:
    ...
    if "PALS" in t:
        return "PALS"
    if "HEARTSAVER" in t or "FIRST AID" in t or t == "FA":
        return "FA"
    return "BLS"

def parse_start(row) -> str:
    """Return 'YYYY-MM-DDTHH:MM' (no tz). Accepts ISO or date+time."""
    ...
    for tfmt in ("%H:%M", "%I:%M%p", "%I%M%p"):
        try:
            tt = datetime.strptime(t_norm, tfmt).strftime("%H:%M")
            break
        except Exception:
            continue
    if not tt:
...
def main():
    # Pick CSV: arg1 OR data/sessions.csv OR sessions.csv
    candidates = []
    if len(sys.argv) > 1:
