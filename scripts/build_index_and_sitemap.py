    ...
    *,
    ...
    category_cards = [
        ...
        },
        {
            ...
            "common": [
                "Unclear requirement",
                {"label": "Course finder", "href": "/#class-finder"},
            ...
            "links": [
                ...
                ("Use course finder", "/#class-finder"),
...
""".rstrip()
    ...
    finder_script = """
<script src="/assets/class-finder-filter.js?v=20260829-1"></script>
<script>ClassFinderFilter.init();</script>
""".rstrip()
...
def write_heartsaver_bridge_course_pages() -> None:
    # Heartsaver course-family pages are generated by scripts/build_slug_hubs.py,
    # where course visibility, suppression, seed offers, and current session
    # matching are already available. Keep this as a no-op so the index builder
    # does not overwrite those richer course hubs with the old bridge pages.
    return None
...
def build():
    ...
    future_sessions = [
        session for session in load_schedule_future_sessions()
        if session.get("_parsed_start") and session["_parsed_start"] > now_dt
    ]
