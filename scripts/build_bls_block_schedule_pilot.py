...
def css() -> str:
    ...
    * { box-sizing: border-box; }
    ...
    .day-timeline { position: absolute; left: 3px; right: 3px; bottom: 3px; height: 6px; display: grid; grid-template-columns: repeat(48, minmax(0, 1fr)); overflow: hidden; border: 1px solid #aeb7c2; border-radius: 999px; background: transparent; }
    .day-timeline-segment { min-width: 0; background: transparent; }
    .day-timeline-segment.is-available { background: #cbd2da; }
    .day-timeline-segment.is-seated { background: #9fb8ca; }
    .day-timeline-segment.is-barnacle { background: #ead47b; }
    .day-timeline-segment.is-anchor { background: #83bd91; }
    .day-button.is-past,
    ...
    .start-grid button.is-seated { background: #f4f8fb; border-color: #9fb8ca; color: #243746; }
    .start-grid button.is-barnacle { background: #fffbea; border-color: #d7bf56; color: #5b4a09; }
    .start-grid button.is-anchor { background: #f2faf4; border-color: #71ad80; color: #194927; }
    .schedule-cue {
      display: inline-flex;
      align-items: center;
      gap: 4px;
    ...
    .course.is-seated { border-left: 3px solid #9fb8ca; background: #f8fbfd; }
    .course.is-anchor { border-left-color: #71ad80; background: #f5faf6; }
    .course.is-anchor .schedule-cue { border-color: #71ad80; background: #edf7ef; color: #194927; }
    .course.is-barnacle { border-left: 3px solid #d7bf56; background: #fffdf5; }
    .course.is-barnacle .schedule-cue { border-color: #d7bf56; background: #fff9df; color: #5b4a09; }
    .selected-summary {
      display: grid;
      gap: 6px;
    }
...
def render_html(payload: dict[str, Any]) -> str:
    ...
    meta_description = html.escape(str(page_config.get("meta_description") or page_config.get("intro") or ""), quote=True)
  ...
  <script>
      ...
      renderedMonthKeys.forEach(monthKey => {{
        ...
        for (let dayNum = 1; dayNum <= last.getDate(); dayNum += 1) {{
          ...
          if (available) {{
            ...
            const segments = Array(48).fill('');
            const seatedLabels = [];
            const anchorLabels = [];
            const barnacleLabels = [];
            const optionLabels = [];
            let hasSeated = false;
            available.startTimes.forEach(slot => {{
              slot.courses.forEach(course => {{
                ...
                const segmentState = role === 'anchor' ? 'is-anchor' : (role === 'barnacle' ? 'is-barnacle' : (isSeated ? 'is-seated' : 'is-available'));
                if (isSeated) {{ hasSeated = true; if (!seatedLabels.includes(slot.displayStartTime)) seatedLabels.push(slot.displayStartTime); }}
                if (role === 'anchor' && !anchorLabels.includes(slot.displayStartTime)) anchorLabels.push(slot.displayStartTime);
                else if (role === 'barnacle' && !barnacleLabels.includes(slot.displayStartTime)) barnacleLabels.push(slot.displayStartTime);
                else if (!isSeated && !optionLabels.includes(slot.displayStartTime)) optionLabels.push(slot.displayStartTime);
                for (let band = startBand; band < endBand; band += 1) {{
                  const priority = {{ '': 0, 'is-available': 1, 'is-seated': 2, 'is-barnacle': 3, 'is-anchor': 4 }};
            ...
            if (hasSeated) {{ const star = document.createElement('span'); star.className = 'day-star'; star.setAttribute('aria-hidden', 'true'); star.textContent = '★'; number.appendChild(star); }}
            const timeline = document.createElement('span');
            timeline.className = 'day-timeline';
            timeline.setAttribute('aria-hidden', 'true');
            segments.forEach(state => {{ const segment = document.createElement('span'); segment.className = 'day-timeline-segment' + (state ? ' ' + state : ''); timeline.appendChild(segment); }});
    ...
    function renderStarts() {{
      ...
      groups.forEach((slots, label) => {{
        ...
        const button = document.createElement('button');
        button.type = 'button';
        const isAnchor = slot.courses.some(course => String(course.schedule_role || course.scheduleRole || '').toLowerCase() === 'anchor');
        const isBarnacle = slot.courses.some(course => String(course.schedule_role || course.scheduleRole || '').toLowerCase() === 'barnacle');
        const isSeated = slot.courses.some(course => course.offerType === 'seated_class');
        button.textContent = (isSeated ? '★ ' : '') + slot.displayStartTime;
        button.classList.add(isAnchor ? 'is-anchor' : (isBarnacle ? 'is-barnacle' : (isSeated ? 'is-seated' : 'is-available')));
    ...
    function renderCourses() {{
      ...
      slot.courses.forEach(course => {{
        ...
        const role = String(course.schedule_role || course.scheduleRole || '').toLowerCase();
        const isSeated = course.offerType === 'seated_class';
        if (isSeated) item.classList.add('is-seated');
        if (role === 'anchor') item.classList.add('is-anchor');
        if (role === 'barnacle') item.classList.add('is-barnacle');
