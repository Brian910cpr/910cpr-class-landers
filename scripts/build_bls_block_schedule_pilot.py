...
def css() -> str:
    ...
    * { box-sizing: border-box; }
    ...
    .day-timeline-segment.is-barnacle { background: #ffe67a; }
    .day-timeline-segment.is-anchor { background: #72c38a; }
    .day-button.is-past,
  ...
  <script>
          ...
          if (available) {{
            ...
            available.startTimes.forEach(slot => {{
              slot.courses.forEach(course => {{
                ...
                if (role === 'anchor') {{ hasAnchor = true; if (!anchorLabels.includes(slot.displayStartTime)) anchorLabels.push(slot.displayStartTime); }}
                else if (!optionLabels.includes(slot.displayStartTime)) optionLabels.push(slot.displayStartTime);
            ...
            if (hasAnchor) {{ const star = document.createElement('span'); star.className = 'day-star'; star.setAttribute('aria-hidden', 'true'); star.textContent = '★'; number.appendChild(star); }}
            ...
            const activityText = [anchorLabels.length ? 'Confirmed class at ' + anchorLabels.join(', ') : '', optionLabels.length ? 'additional options at ' + optionLabels.join(', ') : ''].filter(Boolean).join('; ');
    ...
    function renderStarts() {{
      ...
      groups.forEach((slots, label) => {{
        ...
        button.disabled = disabled;
        button.setAttribute('aria-disabled', String(disabled));
        button.setAttribute('aria-label', slot.displayStartTime + (isAnchor ? ' confirmed class' : isBarnacle ? ' good fit around an existing class' : ' available option') + (disabled ? '; not bookable; past ' + scheduleTimezone + ' start time' : ''));
