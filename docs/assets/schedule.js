...
(() => {
  ...
  const FEED_PATHS = [
    "periscope_full.json",       // same folder as index.html (GitHub Pages docs root)
    "./periscope_full.json",
    "/periscope_full.json",
    "/assets/periscope_full.json" // optional fallback if you ever move it
  ];
  ...
  function detectUrl(it){ const k=Object.keys(it).find(k=>/(url|href|link)/i.test(k)); return (k&&it[k])||""; }
  function detectCity(it, all){
    const direct = it.city || it.City || it.location_city || it.LocationCity;
  ...
  function parseFlexDate(sIn){
    ...
    if (m){ let[,Y,M,D,h,mi,ss,ap,tz]=m; let H=+h; if(ap){ap=ap.toUpperCase(); if(ap==="PM"&&H!==12)H+=12; if(ap==="AM"&&H===12)H=0;}
    ...
    m=s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})(?:\s+(\d{1,2}):(\d{2})(?:\s*(AM|PM))?)?(?:\s*[A-Z]{1,4})?$/i);
    ...
    m=s.match(/(\d{4}-\d{2}-\d{2})(?:[ T](\d{1,2}):(\d{2}))?/); if(m){
      const iso=`${m[1]}T${m[2]?String(m[2]).padStart(2,"0"):"09"}:${m[3]||"00"}:00`; d=new Date(iso); if(!isNaN(d)) return d;
    }
    return null;
  }
  ...
  function setChips(id, items){
    ...
    const html=(items||[]).map(n=>({label:chipLabel(n),url:n.url||"#"})).filter(x=>x.label)
      .map(x=>`<a class="timechip" href="${x.url}">${x.label}</a>`).join("");
  ...
  const tri=f=>({ren:n=>family(n)===f&&isRenew(n)&&!isHeart(n), init:n=>family(n)===f&&!isRenew(n)&&isInPerson(n), heart:n=>family(n)===f&&isHeart(n)});
