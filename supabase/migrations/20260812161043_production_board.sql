...
insert into public.production_board_cards(title,project,lane,value_score,work_score,summary,details,flags,brian_override,manual_rank) values
('BLS selector 3-column comparison matrix','Public BLS Selector','next',8,3,'Make Initial, Renewal, and Skills Testing obvious peers while comparing Classroom and Blended delivery choices across them.','Rebuild the current two-card BLS selector as a responsive 3-column comparison matrix. Top row: one image over each certification-path column (BLS Provider Initial, BLS Provider Renewal, BLS Skills Testing). Row headers at far left: Classroom icon and Blended icon; do not repeat delivery icons inside products. Intended choices: Classroom Initial $75; Classroom Renewal $75; Classroom Skills Testing unavailable; Blended Initial $95; Blended Renewal $95; Blended Skills Testing $55. Entire available cells must be clickable. Unavailable combinations show a simple dash and are not clickable. Preserve existing course IDs, selection state, availability, downstream scheduling behavior, and responsive usability. Conceptual hierarchy: IMAGE → COURSE → DELIVERY METHOD → PRICE. Source: remote/mobile review, August 10, 2026.',array['CUSTOMER IMPACT','REVENUE'],false,null);
