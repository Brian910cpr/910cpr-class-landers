...
begin
  ...
  select count(*) into v_taken from public.landerware_public_order_students s join public.landerware_public_orders o on o.id=s.order_id where o.external_class_id=trim(p_external_class_id) and (o.status='paid' or (o.status in ('held','checkout_open') and o.hold_expires_at>now()));
  if v_inventory.capacity is not null and v_inventory.externally_registered + v_taken + v_requested > v_inventory.capacity then raise exception 'insufficient_seats'; end if;
