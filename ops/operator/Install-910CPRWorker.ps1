...
if ($existingIdentity -and -not $ForceReenroll) {
    ...
    $identity = [pscustomobject][ordered]@{
        ...
        capabilities = @('heartbeat.local','diagnostic.local','github.queue','github.receipt','job.selftest.ping','job.site.healthcheck','job.reject-unapproved','duplicate-suppression.local')
...
$identity | Add-Member -NotePropertyName worker_version -NotePropertyValue '0.2.3' -Force
$identity | Add-Member -NotePropertyName capabilities -NotePropertyValue @('heartbeat.local','diagnostic.local','github.queue','github.receipt','job.selftest.ping','job.site.healthcheck','job.reject-unapproved','duplicate-suppression.local') -Force
